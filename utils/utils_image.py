...
# --------------------------------------------
# get MAT of size HxWxn_channles (RGB)
# --------------------------------------------
def imread_uint(path, n_channels=3):
    #  input: path
    # output: HxWx3(RGB or GGG), or HxWx1 (G)
    if n_channels == 1:
        ...
        if img.ndim == 2:
            ...
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)  # RGB            
# added else for 31 channel HSI, @IITD - 14/02/2023
    else:
        img = scio.loadmat(path)
        img = torch.from_numpy(img['rad'])
        #print(img.shape)
    return img
...
def hsisave(img, img_path):
    img = np.squeeze(img)
    print(img.dtype)
    # 16/02/'23'
    # img = uint2single(img)
    scio.savemat(img_path,{'rad':img})
    # cv2.imwrite(img_path, img)
